...
int lan8820_config_init(struct phy_device *phydev)
{
int phyregloop,rc;
	rc = phy_read(phydev, MII_LAN83C185_CTRL_STATUS);
	if (rc < 0)
	{
		printk(KERN_ALERT "%s :phy_read failed with %x rc\n", __FUNCTION__,rc);
		return rc;
	}

	rc = phy_write (phydev, 0, 0x2000); // override auto mdx for now. force 100M
	if (rc < 0)
	{
		printk(KERN_ALERT "%s :phy_write failed with %x rc\n", __FUNCTION__,rc);
		return rc;
	}
...
EXPORT_SYMBOL_GPL(lan8820_config_init);
...
static struct phy_driver smsc_phy_driver[] = {
{
	...
	.resume		= genphy_resume,
        ...
        .phy_id         = 0x0007c0e0, /* OUI=0x00800f, Model#=0x0d */
        .phy_id_mask    = 0xfffffff0,
        .name           = "SMSC LAN8820 PHY",
